...
DEFAULT_PARLGOV_PARTY_MAP = {
    ...
    "IT": {
        "Aut (SVP, UV, PATT, UPT) - PSI": ["SVP"],
        "Aut (SVP, UV, PATT, UPT)-PSI-MAIE": ["SVP"],
        "Aut (SVP-PATT, UV)": ["SVP"],
        "Art.1-MDP": ["S"],
        "FI-BP": ["FI-PdL"],
        "FI-PdL XVII": ["FI-PdL"],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional party-month CSV with party, month, speech_rows, and speech_words. "
            "When supplied, use it instead of reloading ParlaMint speech volume."
        ),
    )
    return parser
...
def add_external_speech_volume(df: pd.DataFrame, path: Path) -> pd.DataFrame:
    volume = pd.read_csv(path, low_memory=False)
    required = {"party", "month", "speech_rows", "speech_words"}
    missing = sorted(required - set(volume.columns))
    if missing:
        raise ValueError(f"External speech-volume CSV is missing: {missing}")
    party_month = volume[["party", "month", "speech_rows", "speech_words"]].copy()
    party_month = party_month.rename(
        columns={
            "party": "speech_party",
            "speech_rows": "speech_volume_segments_external",
            "speech_words": "speech_volume_words",
        }
    )
    month_total = (
        party_month.groupby("month", as_index=False)
        .agg(
            month_total_speech_segments=("speech_volume_segments_external", "sum"),
            month_total_speech_words=("speech_volume_words", "sum"),
        )
    )
    out = df.merge(party_month, on=["speech_party", "month"], how="left")
    out = out.merge(month_total, on="month", how="left")
    out["speech_volume_segments"] = out["speech_volume_segments_external"].fillna(
        out["speech_rows"]
    )
    out["speech_volume_words"] = out["speech_volume_words"].fillna(0)
    out["log1p_speech_segments"] = np.log1p(out["speech_volume_segments"])
    ...
    out["speech_word_share"] = out["speech_volume_words"] / out["month_total_speech_words"]
    return out.drop(columns=["speech_volume_segments_external"])
...
def build_regression_panel(
    ...
    parlgov_db: Path,
    parlgov_country_short: str,
    speech_volume_input: Path | None = None,
) -> pd.DataFrame:
    ...
    panel = (
        add_external_speech_volume(panel, speech_volume_input)
        if speech_volume_input is not None
        else add_speech_volume(panel, country_code)
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    panel = build_regression_panel(
        ...
        speech_volume_input=args.speech_volume_input,
    )
